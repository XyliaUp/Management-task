    ...
    public partial class Users : UserControl
    {
        ...
        private void radioButton1_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                if (radioButton1.Checked)
                {
                    ...
                    {
                        dataGridView1.Rows.Add(user[i].UserID, user[i].UserName,
        ...
        private void radioButton4_CheckedChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    List<LTS.User> user = new List<LTS.User>();
                    user = DAT.DataAccess.GetUser().Where(s => !s.UserActivated).ToList();//list from db
                    for (int i = 0; i < user.Count; i++)
                    {
                        dataGridView1.Rows.Add(user[i].UserID, user[i].UserName,
                            user[i].UserEmail, user[i].UserAdmin, user[i].UserActivated);
        ...
        private void saveFileDialog1_FileOk(object sender, CancelEventArgs e)
        {
            try
            {
                ...
                Paragraph writing = new iTextSharp.text.Paragraph("道路运输管理信息系统 (TMS)" + Environment.NewLine + "Users Information: " + what + "                " + DateTime.Now.ToString() + Environment.NewLine + Environment.NewLine);
